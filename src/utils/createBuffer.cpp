                      ...
                      int bitWidth)
{
    ...
    // For example, if a float tensor of dimension 2x4x3 is tightly packed in a buffer of 96 bytes,
    // then the strides would be (48,12,4)
    ...
    if (isTfNBuffer)
    {
        ...
        printf("stepEquivalentTo0 : %d , quantizedStepSize : %f\n", stepEquivalentTo0, quantizedStepSize);
        ...
        // userBufferEncoding = std::unique_ptr<zdl::DlSystem::UserBufferEncodingTfN>(new zdl::DlSystem::UserBufferEncodingTfN(0, 0.003921519965, bitWidth));
    }
    else
    {
    ...
    snpeUserBackedBuffers.push_back(ubFactory.createUserBuffer(applicationBuffers.at(name).data(),
                                                               bufSize, strides, userBufferEncoding.get()));
